...
function M.setup(ctx)
  ...
  local function refresh_monitor_reserved_cache(delay)
    ...
    local command = string.format(
      [=[sleep %.2f; cache="${XDG_RUNTIME_DIR:-/tmp}/hyprland-monitor-reserved.tsv"; tmp="$cache.tmp"; %s -j monitors 2>/dev/null | %s -r '.[] | [.name, .x, .y, .width, .height, .scale, .reserved[0], .reserved[1], .reserved[2], .reserved[3]] | @tsv' > "$tmp" && mv "$tmp" "$cache"]=],
      as_number(delay, 0),
      shell_quote(hyprctl_command),
      shell_quote(jq_command)
    )
